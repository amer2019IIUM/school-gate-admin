    ...
    <div v-else>
      ...
      <v-data-table :headers="headers" :items="teachers" class="elevation-1"
        ><template v-slot:item.gender="{ item }">
          <td v-if="item.gender == 'MALE'">معلم</td>
          <td v-if="item.gender == 'FEMALE'">معلمة</td>
        </template>
        <template v-slot:top>
          <v-toolbar flat>
        ...
        <template v-slot:item.actions="{ item }">
          <v-icon small class="mr-2" @click="editTeacher(item)" color="blue">
            mdi-pencil
          </v-icon>
          <v-icon small @click="deleteTeacher(item)" color="red darken-4">
            mdi-delete
          </v-icon>
        </template>
      </v-data-table>
...
<script>
import Teachers from "../../graphql/teachers/queries/teachers.gql";
export default {
  data: () => ({
    dialog: false,
    dialogDelete: false,
    headers: [
      { text: "رقم الهوية", value: "passport" },
      {
        text: "اسم المعلم",
        align: "start",
        sortable: false,
